...
function normalizeMediaItems(items) {
  return parseJsonField(items)
    ...
    .filter((item) => hasUsableText(item.title) || hasUsableText(item.url) || hasUsableText(item.imageUrl))
}
...
function normalizeResourceLinks(links) {
  return parseJsonField(links)
    ...
    .filter((link) => hasUsableText(link.label) && hasUsableText(link.url) && !isTodoValue(link.url))
}
...
function normalizeRegistrationLinks(raw) {
  ...
  if (items.length > 0) {
    return items.filter((link) => hasUsableText(link.title) && hasUsableText(link.url) && !isTodoValue(link.url))
  }
...
export function normalizeProgram(raw) {
  ...
  const placeholderDetails = parseJsonField(raw.placeholderDetails ?? raw.placeholder_details, []).filter(
    (line) => hasUsableText(line) && !isTodoValue(line),
  )
  ...
  return {
    ...
    interestFormLabel: legacyInterest?.title || raw.interestFormLabel || raw.interest_form_label || '',
    ...
    displayOrder: typeof raw.displayOrder === 'number' ? raw.displayOrder : raw.display_order ?? 999,
...
export async function saveProgram(program) {
  ...
  const { data, error } = await supabase.from('programs').upsert(row, { onConflict: 'slug' }).select().single()
...
export function getProgramOverviewText(program, fallbackText) {
  ...
  return fallbackText || 'More information about this program will be added as details become available.'
}
