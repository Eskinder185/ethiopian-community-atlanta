...
export function normalizeLeadershipMember(raw) {
  if (!raw) return null

  const id = raw.id || raw.slug || ''
  const name = raw.name || ''
  if (!hasUsableText(name)) return null
...
export function isLeadershipMemberVisible(member) {
  ...
  if (member.active === false || member.visible === false || member.showOnLeadershipPage === false) {
    return false
  }
...
export function getMemberImageAlt(member) {
  if (hasUsableText(member?.imageAlt)) return member.imageAlt.trim()
  if (hasUsableText(member?.photo?.alt)) return member.photo.alt.trim()
  if (hasUsableText(member?.name)) return `Photo of ${member.name}`
  return 'ECAA leadership photo'
}
  ...
  return LEADERSHIP_COMMITTEES.map((committee) => ({
    ...
    members: sortLeadershipMembers(
      visibleMembers.filter((member) => {
        ...
        return definition ? definition.id === committee.id : committeeKey(member.committee) === committeeKey(committee.title)
      }),
    ),
  }))
}
...
export async function saveLeadershipMember(member) {
  ...
  const { data, error } = await supabase.from('leadership').upsert(row).select().single()
