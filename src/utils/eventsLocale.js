...
export function applyEventItemLocale(item, language = "en") {
  ...
  return {
    ...
    excerpt: hasUsableText(overlay.excerpt)
      ...
      : hasUsableText(overlay.shortDescription)
        ? overlay.shortDescription.trim()
    ...
    imageAlt: hasUsableText(overlay.imageAlt)
      ? overlay.imageAlt.trim()
      : hasUsableText(overlay.image_alt)
        ? overlay.image_alt.trim()
        : item.imageAlt,
