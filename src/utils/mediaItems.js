...
function warnSupabaseFallback(context, error) {
  console.warn(`Media items could not be loaded from Supabase. Using fallback media. (${context})`, error)
}
...
export async function fetchMediaItems(events = []) {
  ...
  try {
    ...
    return merged.length > 0 ? merged.sort((a, b) => a.displayOrder - b.displayOrder) : getFallbackMediaItems(events)
  } catch (error) {
    warnSupabaseFallback('fetchMediaItems', error)
    return getFallbackMediaItems(events)
  }
...
export async function saveMediaItem(item) {
  ...
  if (isValidUuid(item.id)) {
    const { data, error } = await supabase.from('media_items').update(row).eq('id', item.id).select().single()
