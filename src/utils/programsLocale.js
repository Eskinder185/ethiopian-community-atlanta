...
export function getProgramContentAmSource(program) {
  ...
  return program.content_am ?? program.contentAm ?? programsAmharicFallbackBySlug[program.slug] ?? null
}
...
export function buildContentAmFromDraft(draft = {}) {
  ...
  const payload = {
    ...
    short_description: source.short_description ?? source.shortDescription ?? existing.shortDescription ?? '',
    full_description: source.full_description ?? source.fullDescription ?? existing.fullDescription ?? '',
    ...
    placeholder_details: source.placeholder_details ?? source.placeholderDetails ?? existing.placeholderDetails ?? [],
    detail_sections: source.detail_sections ?? source.detailSections ?? existing.detailSections ?? [],
    ...
    registration_links: source.registration_links ?? source.registrationLinks ?? existing.registrationLinks ?? [],
