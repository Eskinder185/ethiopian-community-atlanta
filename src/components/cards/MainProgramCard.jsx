...
export default function MainProgramCard({ program }) {
  ...
  const detailPath =
    program.buttonLink?.startsWith('/') ? program.buttonLink : `/programs/${program.slug || program.id}`
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-2 text-xl font-semibold normal-case tracking-tight text-ecaa-green-950">{program.title}</h3>
      {program.subtitle && (
      ...
      {details.length > 0 && (
        <ul className="mt-5 space-y-2 border-t border-ecaa-border/60 pt-5">
          {details.slice(0, 3).map((line) => (
            <li key={line} className="flex items-start gap-2 text-sm leading-relaxed text-ecaa-ink-subtle">
              <span
