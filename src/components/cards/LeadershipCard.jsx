...
export default function LeadershipCard({ member, committee }) {
  ...
  const hasPhoto = Boolean(imageSrc && !imageSrc.startsWith('TODO'))
  const socials = getValidSocials(member.socials)
  const altText = member.photo?.alt || `Photo of ${member.name}`
  ...
  return (
    <article className="ecaa-card-hover group flex h-full flex-col overflow-hidden p-0">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        {socials.length > 0 && (
          <ul className="mt-5 flex flex-wrap gap-2" aria-label={`Social links for ${member.name}`}>
            {socials.map((social) => (
