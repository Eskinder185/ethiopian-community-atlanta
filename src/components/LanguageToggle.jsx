...
export default function LanguageToggle({ variant = 'desktop' }) {
  ...
  if (variant === 'mobile') {
    return (
      <div className="rounded-lg border border-ecaa-border/70 bg-ecaa-cream/40 px-4 py-3">
        <p className="text-sm font-medium text-ecaa-ink">
          {t('common.language')}:{' '}
          <button
            type="button"
            className={`font-semibold ${language === 'en' ? 'text-ecaa-green-900' : 'text-ecaa-ink-muted hover:text-ecaa-green-800'}`}
          ...
          <button
            type="button"
            className={`font-semibold ${language === 'am' ? 'text-ecaa-green-900' : 'text-ecaa-ink-muted hover:text-ecaa-green-800'}`}
