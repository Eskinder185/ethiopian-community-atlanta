...
export default function EmptyState({
  ...
  const safeDescription =
    typeof description === 'string' && !isTodoValue(description) && !description.includes('homepage.json')
      ? description
  ...
  return (
    ...
    >
      {showEyebrow && (
        <p className="text-sm font-semibold uppercase tracking-widest text-ecaa-gold-600">{eyebrow}</p>
      )}
      <HeadingTag className={`${showEyebrow ? 'mt-3' : ''} ${headingClasses[headingLevel]}`}>
        {title}
      </HeadingTag>
      <p className="mt-3 text-base leading-relaxed text-ecaa-ink-muted sm:text-lg">{safeDescription}</p>
