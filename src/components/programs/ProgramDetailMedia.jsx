...
const FLYER_PLACEHOLDER_TEXT =
  'Program flyer will appear here once uploaded by an ECAA editor.'
    ...
    <figure className="overflow-hidden rounded-ecaa-xl border border-ecaa-border/80 bg-ecaa-white shadow-ecaa-sm">
      ...
      {(item.title || item.caption) && (
        <figcaption className="border-t border-ecaa-border/60 px-4 py-3">
          {item.title && <p className="font-semibold normal-case text-ecaa-green-950">{item.title}</p>}
          {item.caption && <p className="mt-1 text-sm leading-relaxed text-ecaa-ink-muted">{item.caption}</p>}
        </figcaption>
      )}
    </figure>
  )
...
function MediaLinkCard({ item, label = 'Open' }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-ecaa-xl border border-ecaa-border/80 bg-ecaa-white p-6 shadow-ecaa-sm">
      {item.title && <h3 className="text-lg font-semibold normal-case text-ecaa-green-950">{item.title}</h3>}
...
export default function ProgramDetailMedia({ program, labels = {} }) {
  ...
  const emptyDescription = program.mediaEmptyMessage || labels.mediaEmpty || 'Media for this program will be added soon.'
