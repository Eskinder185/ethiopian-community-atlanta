          ...
          {links.length > 0 ? (
            <ul className="mt-8 space-y-4">
              {links.map((link) => {
                ...
                const buttonLabel = link.buttonLabel || (isInternal ? 'Open Resource' : 'Open Link')
                  ...
                  <li key={link.id}>
                    <article className="rounded-ecaa-xl border border-ecaa-border/80 bg-ecaa-white p-5 shadow-ecaa-sm sm:p-6">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          <h3 className="text-lg font-semibold normal-case text-ecaa-green-950">{link.label}</h3>
                          {link.description && (
                        ...
                        {isInternal ? (
                          <CTAButton to={link.url} variant="secondary" size="sm" className="shrink-0">
                            {buttonLabel}
                          </CTAButton>
                        ) : (
                          <CTAButton
            ...
            <div className="mt-8">
              <EmptyState
                ...
                description={labels.resourcesEmpty || 'Helpful links and resources will be added soon.'}
              />
            </div>
          )}
        </div>
