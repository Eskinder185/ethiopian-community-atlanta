export default function ChatLauncher({ isOpen, onClick }) {
  return (
    <button
      ...
      aria-label={isOpen ? 'Close ECAA Helper' : 'Open ECAA Helper — questions about membership, events, and programs'}
      onClick={onClick}
    >
      <span aria-hidden="true" className="flex h-7 w-7 items-center justify-center rounded-full bg-ecaa-gold-500/20 text-sm">
        ?
      </span>
      <span className="hidden sm:inline">Questions? Ask ECAA</span>
      <span className="sm:hidden">Ask ECAA</span>
    </button>
  )
}
