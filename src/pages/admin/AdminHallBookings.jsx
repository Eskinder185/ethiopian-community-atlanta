    ...
    return (
      <div className="mx-auto max-w-4xl">
        ...
        <h1 className="mt-2 text-2xl font-semibold text-ecaa-ink">
          {adminT("hallBookings.requestTitle")}
        </h1>
        ...
        <div className="mt-6 rounded-ecaa-xl border border-ecaa-border/80 bg-ecaa-cream/30 p-5 text-sm">
          <p>
            <strong>{adminT("hallBookings.requester")}:</strong> {draft.requesterName}
          </p>
          <p className="mt-1">
            <strong>{adminT("hallBookings.email")}:</strong> {draft.requesterEmail}
          </p>
          <p className="mt-1">
            <strong>{adminT("hallBookings.phone")}:</strong> {draft.requesterPhone || "—"}
          </p>
          <p className="mt-1">
            <strong>{adminT("hallBookings.event")}:</strong> {draft.eventTitle}
          </p>
          <p className="mt-1">
            <strong>{adminT("hallBookings.eventType")}:</strong> {draft.eventType}
          </p>
          <p className="mt-1">
            <strong>{adminT("hallBookings.table.start")}:</strong>{" "}
            {formatBookingDateTime(draft.startTime)}
          </p>
          <p className="mt-1">
            <strong>End:</strong> {formatBookingDateTime(draft.endTime)}
          </p>
          <p className="mt-1">
            <strong>{adminT("hallBookings.guests")}:</strong> {draft.expectedGuests || "—"}
          </p>
          {draft.notes && (
            <p className="mt-3 whitespace-pre-wrap">
              <strong>{adminT("hallBookings.notes")}:</strong> {draft.notes}
            </p>
          )}
        </div>
        ...
        <form onSubmit={handleSave} className="mt-6 space-y-4">
          ...
          <FormInput
            id="public-title"
            label={adminT("hallBookings.publicTitle")}
            value={draft.publicTitle}
            onChange={(e) => updateDraft("publicTitle", e.target.value)}
            hint={adminT("hallBookings.publicTitleHint")}
          />
          <ToggleField
            id="visible-public"
            label={adminT("hallBookings.showOnCalendar")}
            checked={draft.visiblePublic === true}
            onChange={(v) => updateDraft("visiblePublic", v)}
          />
          <FormTextarea
            id="admin-notes"
            label={adminT("hallBookings.adminNotes")}
            rows={4}
          ...
          <tbody className="divide-y divide-ecaa-border/60">
            {filtered.length === 0 ? (
              ...
              filtered.map((booking) => (
                <tr key={booking.id}>
                  <td className="px-4 py-3 font-medium text-ecaa-green-950">
                    {booking.eventTitle}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <StatusBadge
                      ...
                      variant={booking.status === "approved" ? "live" : "soon"}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <button
