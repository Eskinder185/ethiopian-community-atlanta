...
function ToggleField({ id, label, checked, onChange, hint }) {
  return (
    <label htmlFor={id} className="flex cursor-pointer items-start gap-3 rounded-lg border border-ecaa-border/80 bg-ecaa-cream/40 px-4 py-3">
      <input
        id={id}
        type="checkbox"
        checked={checked}
...
export default function AdminPrograms() {
  ...
  const updateContentAmSection = (index, key, value) => {
    setDraft((current) => {
      ...
      return { ...current, content_am: { ...(current.content_am || {}), detail_sections: sections } }
    })
  }
  ...
  const addContentAmSection = () => {
    setDraft((current) => ({
      ...
      content_am: {
        ...(current.content_am || {}),
        detail_sections: [...(current.content_am?.detail_sections ?? []), { heading: '', body: '' }],
      },
    }))
  }
  ...
  const handleSave = async (event) => {
    ...
    const payload = {
      ...
      resourceLinks: (draft.resourceLinks ?? []).filter((link) => link.label?.trim() && link.url?.trim()),
    ...
    return (
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold tracking-tight text-ecaa-ink">
              {selectedSlug === '__new__' ? adminT('programs.addProgram') : adminT('programs.editProgram')}
            </h1>
          </div>
          {draft.slug && (
            <StatusBadge
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          {editorLang === 'en' ? (
          <>
          <SectionCard title={adminT('programs.basics')} description={adminT('programs.basicsHelper')}>
          ...
          <SectionCard
            ...
            action={
              <button type="button" onClick={addPlaceholderLine} className="btn btn-secondary btn-sm">
          ...
          <SectionCard
            ...
            action={
              <button type="button" onClick={addDetailSection} className="btn btn-secondary btn-sm">
          ...
          >
            <div className="space-y-4">
              {(draft.detailSections ?? []).map((section, index) => (
                <div key={`section-${index}`} className="rounded-lg border border-ecaa-border/70 bg-ecaa-cream/30 p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-ecaa-green-950">
                      {adminT('programs.sectionN').replace('{n}', index + 1)}
                    </p>
                    <button type="button" onClick={() => removeDetailSection(index)} className="btn btn-ghost btn-sm">
          ...
          >
            <div className="space-y-5">
              {(draft.mediaItems ?? []).map((item, index) => (
                <div key={item.id || `media-${index}`} className="rounded-lg border border-ecaa-border/70 bg-ecaa-cream/30 p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-ecaa-green-950">
                      {adminT('programs.mediaItemN').replace('{n}', index + 1)}
                    </p>
                    <button type="button" onClick={() => removeMediaItem(index)} className="btn btn-ghost btn-sm">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <FormInput
                      ...
                      value={item.imageUrl}
                      onChange={(event) => updateMediaItem(index, 'imageUrl', event.target.value)}
          ...
          <SectionCard
            ...
            action={
              <button type="button" onClick={addResourceLink} className="btn btn-secondary btn-sm">
          ...
          >
            <div className="space-y-5">
              {(draft.resourceLinks ?? []).map((link, index) => (
                <div key={link.id || `resource-${index}`} className="rounded-lg border border-ecaa-border/70 bg-ecaa-cream/30 p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-ecaa-green-950">
                      {adminT('programs.resourceN').replace('{n}', index + 1)}
                    </p>
                    <button type="button" onClick={() => removeResourceLink(index)} className="btn btn-ghost btn-sm">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <FormInput
                      ...
                      onChange={(event) => updateResourceLink(index, 'buttonLabel', event.target.value)}
                    ...
                    <FormTextarea
                      ...
                      onChange={(event) => updateResourceLink(index, 'description', event.target.value)}
          ...
          <SectionCard
            ...
            action={
              <button type="button" onClick={addRegistrationLink} className="btn btn-secondary btn-sm">
          ...
          >
            <div className="mb-4 space-y-4">
              <FormTextarea
                ...
                onChange={(event) => updateDraft('registrationEmptyMessage', event.target.value)}
                ...
                >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <FormInput
                      ...
                      label={adminT('forms.title')}
                      value={link.title}
                      onChange={(event) => updateRegistrationLink(index, 'title', event.target.value)}
                    />
                    <FormInput
                      id={`registration-button-${index}`}
                      label={adminT('forms.buttonLabel')}
                      value={link.buttonLabel}
                      onChange={(event) => updateRegistrationLink(index, 'buttonLabel', event.target.value)}
                    />
                    <FormInput
                      id={`registration-url-${index}`}
                      label={adminT('forms.url')}
                      value={link.url}
                      onChange={(event) => updateRegistrationLink(index, 'url', event.target.value)}
                      className="sm:col-span-2"
                    />
                    <FormTextarea
                      ...
                      rows={3}
                      value={link.description}
                      onChange={(event) => updateRegistrationLink(index, 'description', event.target.value)}
                      className="sm:col-span-2"
                    />
                    <FormTextarea
                      ...
                      rows={2}
                      value={link.note}
                      onChange={(event) => updateRegistrationLink(index, 'note', event.target.value)}
          ...
          <SectionCard title={adminT('programs.programsPageCard')}>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <FormInput
                ...
                onChange={(event) => updateDraft('buttonLink', normalizeWebsiteRoute(event.target.value))}
            ...
            <>
                  ...
                  {getContentAmList('placeholder_details').map((line, index) => (
                    <div key={`am-placeholder-${index}`} className="flex gap-2">
                      <FormInput
                        ...
                        value={line}
                        onChange={(event) => updateContentAmLine('placeholder_details', index, event.target.value)}
                        className="flex-1"
                      />
                      <button
                        type="button"
              ...
              <SectionCard
                ...
                action={
                  <button type="button" onClick={addContentAmSection} className="btn btn-secondary btn-sm">
                    {adminT('programs.addDetailSection')}
                  </button>
                }
              >
                <div className="space-y-4">
                  {getContentAmList('detail_sections').map((section, index) => (
                    <div key={`am-section-${index}`} className="rounded-lg border border-ecaa-border/70 bg-ecaa-cream/30 p-4">
                      <div className="mb-3 flex items-center justify-between gap-2">
                        <p className="text-sm font-semibold text-ecaa-green-950">
                          {adminT('programs.sectionN').replace('{n}', index + 1)}
                        </p>
                        <button type="button" onClick={() => removeContentAmSection(index)} className="btn btn-ghost btn-sm">
                          {adminT('programs.removeDetailSection')}
                        </button>
                      </div>
                      <div className="space-y-3">
                        <FormInput
                          ...
                          label={adminT('forms.heading')}
                          value={section.heading || ''}
                          onChange={(event) => updateContentAmSection(index, 'heading', event.target.value)}
                        />
                        <FormTextarea
                          ...
                          rows={3}
                          value={section.body || ''}
                          onChange={(event) => updateContentAmSection(index, 'body', event.target.value)}
                        />
                      </div>
                    </div>
                  ))}
              ...
              <SectionCard title={adminT('programs.amharicEmptyStates')}>
                <div className="space-y-4">
                  <FormTextarea
                    ...
                    onChange={(event) => updateContentAm('registration_empty_message', event.target.value)}
                  />
                  <FormTextarea
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-ecaa-ink">{adminT('programs.title')}</h1>
              ...
              <tr key={program.slug} className="hover:bg-ecaa-cream/30">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-2">
                    <StatusBadge
                      label={program.visible !== false ? adminT('common.visible') : adminT('common.hidden')}
                      variant={program.visible !== false ? 'live' : 'soon'}
                    />
                    {program.featured && <StatusBadge label={adminT('common.featured')} variant="live" />}
                  </div>
                </td>
                <td className="px-4 py-3">
                  <button
