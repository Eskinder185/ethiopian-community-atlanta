...
export default function AdminLeadership() {
  ...
  const handleSave = async (event) => {
    ...
    try {
      if (!hasSupabaseConfig()) {
        throw new Error('Supabase is not configured. Leadership members cannot be saved online yet.')
      }
      const saved = await saveLeadershipMember(payload)
  ...
  const handleDelete = async () => {
    ...
    try {
      if (!hasSupabaseConfig()) {
        throw new Error('Supabase is not configured. Leadership members cannot be deleted online yet.')
      }
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-ecaa-ink">{adminT('leadership.title')}</h1>
              ...
              <li key={member.id}>
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {!member.visible && <StatusBadge label={adminT('common.hidden')} variant="draft" />}
                    {member.featured && <StatusBadge label={adminT('common.featured')} variant="live" />}
                  </div>
                </button>
              </li>
            ))}
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              >
                <div className="mx-auto max-w-xs">
                  {previewMember && (
                    <LeadershipBubble member={previewMember} committee={previewMember.committee} preview />
                  )}
                </div>
              </SectionCard>
              ...
              <SectionCard title={adminT('leadership.profileDetails')}>
                {editorLang === 'en' ? (
                  ...
                  <>
                    <p className="mb-4 text-sm text-ecaa-ink-muted">{adminT('leadership.amharicHelper')}</p>
                    <div className="grid gap-4 sm:grid-cols-2">
                      <FormInput
