...
export const supportPageContent = {
  ...
  am: {
    hero: {
      ...
      description: "ECAAን በልገሳ፣ በስፖንሰርነት፣ በበጎ ፈቃደኝነት እና በማህበረሰብ ፕሮግራሞች ውስጥ በመሳተፍ ይደግፉ።",
      buttons: [
    ...
    finalCta: {
      ...
      description: "ስለ ልገሳ፣ የገንዘብ ማሰባሰቢያ፣ የስፖንሰርሺፕ እና የበጎ ፈቃደኝነት እድሎች ጥያቄዎች ካሉዎት ECAAን ያነጋግሩ።",
      buttons: [
...
function mergeDonationLevels(englishLevels = [], amharicLevels = []) {
  ...
  return englishLevels.map((level, index) =>
    mergeLocalizedContent(level, amharicLevels[index] || {})
  );
}
...
export function getSupportPageContent(language = "en", remoteContent = null) {
  ...
  const donation = {
    ...
    form: {
      ...mergeLocalizedContent(
        mergeLocalizedContent(en.donation.form, am.donation.form),
        remoteAm.donation?.form
      ),
      formUrl: donationFormUrl,
    },
  };
  ...
  return {
    ...
    otherOptions: {
      ...mergeLocalizedContent(
        mergeLocalizedContent(en.otherOptions, am.otherOptions),
        remoteAm.otherOptions
      ),
      links: mergeLinks(
